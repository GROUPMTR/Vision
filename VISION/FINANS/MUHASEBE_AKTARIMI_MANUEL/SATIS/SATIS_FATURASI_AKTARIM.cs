    ...
    public partial class SATIS_FATURASI_AKTARIM : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void DATA_LOAD_BEKLEMEDE()
        {
           ...
           { 
               ...
               { SlctQu = "  SELECT CASE WHEN EINVOICE = 1 THEN 'e-fatura' else  'e-arşiv' END AS FATURANIN_TURU , CASE WHEN TYPE = 8 THEN 'SATIŞ' WHEN TYPE = 3 THEN 'SATIŞ İADE'  END AS TIPI ,* FROM  dbo.FTR_LG_INVOICE  where  (SIRKET_KODU=@SIRKET_KODU) AND  (TYPE = 8 or TYPE = 3) AND (AKTARIM_DURUMU ='BEKLEMEDE' or AKTARIM_DURUMU ='AKTARILMADI') "; } 
               else 
        ...
        private void BTN_FATURA_KONTROL_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
        ...
        private void BTN_EXPORT_EXCEL_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            sfd.FileName = "SATIŞ_BEKLEMEDE.xlsx";
        ...
        {
            if (dr == null) return;
            SATIS_FATURASI_EDIT edt = new SATIS_FATURASI_EDIT(dr["GUID"].ToString(),dr["ID"].ToString());
            edt.ShowDialog();
        ...
        private void BTN_LOGO_ROW_AKTAR_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
        ...
        private void BTN_LOGO_AKTARIMI_DURDUR_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            BTN_LOGO_ROW_AKTAR.Enabled = true;
        } 
        ...
        {  
            ...
            if (rd.BTN_TYPE != "VAZGEC")
            {
                BTN_LOGO_ROW_AKTAR.Enabled = false; 
        ...
        private void LOGO_AKATARIMI_BASLAT(string _DATE)
        {
            int[] selectedRows = gridView_LIST.GetSelectedRows();
            for (int ix = 0; ix <= selectedRows.Length - 1; ix++)
            {
                //Sales.Sales_Dispatches Sdp = new Sales.Sales_Dispatches(); 
                //if (_E_FATURA_TYPE != sdp._E_FATURA_KONTROL(dr["CODE"].ToString()))
                //{
                ...
                FINANS.MUHASEBE_AKTARIMI_MANUEL.SATIS._SATIS_FATURASI sdp = new FINANS.MUHASEBE_AKTARIMI_MANUEL.SATIS._SATIS_FATURASI();
                DataRow dr = gridView_LIST.GetDataRow(selectedRows[ix]);
                _E_FATURA_TYPE = sdp._E_FATURA_KONTROL(dr["CODE"].ToString());
              

                string FATURA_VARYOK_KONTROL = sdp._FATURA_VAR_YOK_KONTROL(dr["NUMBER"].ToString());
                string FATURA_AKTARIM_DURUMU = "";
                if (FATURA_VARYOK_KONTROL == string.Empty)
                {
                    FATURA_AKTARIM_DURUMU = sdp._Insert(_E_FATURA_TYPE, dr["NUMBER"].ToString(), _DATE, _TEMEL_TICARI, _FATURA_SERISI, dr["CODE"].ToString(), _GLOBAL_PARAMETERS._SIRKET_NO);
                }
             ...
             for (int ix = 0; ix <= selectedRows.Length - 1; ix++)
             {
                 ...
                 using (SqlConnection myConnections = new SqlConnection(_GLOBAL_PARAMETERS._CONNECTIONSTRING_MDB))
                 {
                     ...
                     using (SqlCommand myCmd = new SqlCommand())
                     {
                         ...
                         myCmd.Parameters.AddWithValue("@SIRKET_KODU", _GLOBAL_PARAMETERS._SIRKET_KODU);
                         myCmd.Parameters.AddWithValue("@INVOICE_REF_ID", dr["ID"]);
        ...
        private void SATIS_FATURASI_AKTARIM_Deactivate(object sender, EventArgs e)
        {
            timers.Start();
        }

        private void SATIS_FATURASI_AKTARIM_Activated(object sender, EventArgs e)
        {
            timers.Stop ();
        }

        private void SATIS_FATURASI_AKTARIM_FormClosing(object sender, FormClosingEventArgs e)
        {
            timers.Stop();
        } 
